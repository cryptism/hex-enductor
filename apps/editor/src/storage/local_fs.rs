...
/// The old "type"-discriminated format (what the former TypeScript
/// editor's browser-folder storage wrote) doesn't parse here; say how
/// to convert it rather than just failing.
fn parse_error_hint(text: &str) -> &'static str {
    ...
    if old_dialect {
        " — this looks like the old \"type:\"-style project format; convert it with \
         `cargo run -p hexen-cli -- migrate <file>`"
    } else {
        ""
    }
}
