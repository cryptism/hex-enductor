...
export interface FollowViewEvent {
  locationId: string;
  x: number;
  y: number;
  zoom: number;
 ...
 *
 * `ping`/`onPing` and `followView`/`onFollowView` are the exception to
 * all of that: purely ephemeral, never touch project state, never
 * persisted — hexend just re-broadcasts each verbatim to every socket
 * watching the same session, sender included.
 *
 * A read-only consumer (the presentation app) is exactly this same
 * type, minus ever calling `execute`/`undo`/`redo`/`ping`/`followView`
 * — there's no separate read-only variant to keep in sync.
 */
export interface LiveSession {
  ...
  onFollowView(onFollowView: (view: FollowViewEvent) => void): () => void;
...
// union: `state`/`ping`/`followView` are just this message's fields (a
// proto oneof, but that's invisible in JSON — only one is ever present
// at a time). `state`'s own oneof-shaped fields still need
// wireFormat.ts's conversion; `ping`/`followView`'s fields are all plain.
interface WireServerMessage {
  state?: Parameters<typeof openedProjectDataFromWire>[0];
  ping?: PingEvent;
  followView?: FollowViewEvent;
}

function isWireServerMessage(value: unknown): value is WireServerMessage {
  return (
    typeof value === "object" && value !== null && ("state" in value || "ping" in value || "followView" in value)
  );
}
...
export function connectLiveSession(serverUrl: string, path: string): Promise<LiveSession> {
  return new Promise((resolve, reject) => {
    ...
    // returns. A "ping"/"followView" can never arrive first in
    // practice, but is handled defensively (ignored) if it somehow did.
    socket.addEventListener("message", (evt) => {
      ...
      if (message.followView) {
        for (const listener of followViewListeners) listener(message.followView);
        return;
      }
      ...
      if (!settled) {
        ...
        resolve({
          ...
          followView(locationId, x, y, zoom) {
            socket.send(JSON.stringify({ followView: { locationId, x, y, zoom } }));
          },
          undo() {
